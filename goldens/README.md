...
| `parser-behaviors.txt` | 42 accept/reject cases with cited line numbers and event summaries | error *texts* are informative only; accept/reject + line numbers are the contract; two cases expected-to-diverge by decided posture (SPEC §5.10): `bars 0` and negative `vel` — the implementation rejects both |
